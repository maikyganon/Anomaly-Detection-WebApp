...
CLI::CLI(DefaultIO* dio, bool is_simple): dio(dio){
    this->cmdsArrSize = 5;
    this->cmdsArr = new Command*[this->cmdsArrSize]();
    //Point** pArr = new Point*[(*this).v->size()]();
    helperCreateAndInsertCmdsToArr(is_simple);
}

void CLI::setIO(DefaultIO* newDio){
    this->dio=newDio;
    delete ((this->cmdsArr)) ;
    delete ((this->cmdsArr+1));
    delete (this->cmdsArr+2);
    delete (this->cmdsArr+3);
    delete ((this->cmdsArr+4));
...
void CLI::helperCreateAndInsertCmdsToArr(bool is_simple){
    this->hAD = new HybridAnomalyDetector(is_simple);
