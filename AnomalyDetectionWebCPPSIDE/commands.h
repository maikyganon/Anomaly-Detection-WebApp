...
public:
    ...
    virtual void write(float f){
        /*char* oneBptr = (char*) &f;
        int byteswritten = send(fd, oneBptr, sizeof(float),0);
        if (byteswritten<0) {
    ...
    virtual void read(float* f){
        //this function purpose is to read recv bytres into the buffer and then fill f* with the correct floar numbers
        //this function will be used if the data that sohuld be sent will be floats[];

        //In reality this function is used only for reading float for the menu options
        //that is why we care about only values of 1-6;

        /*int devCharToInt = 48;
        int temp;
        char buffer[2];
        int byteread = ::read(fd,buffer,2); //te flag==0 should be verified;
        if (byteread<=0){
            if (byteread<0) {
                throw "reading failure";
            }
            return; //should be verified what the return value should be in this case/
        }
        if (buffer[1]=='\n') {
            temp = buffer[0] - 48;
            f[0] = (float) temp;
...
public:
    ...
    void execute(){
        //writeStringToClient("Anomalies:.\n");
        string respond = "Anomalies:.\n";
        vector<AnomalyReport>::iterator it;
        for (it = (*c->aR).begin(); it != (*c->aR).end(); ++it){
            respond+= to_string((*it).timeStep) + "\t" + (*it).description +"\n";
        }
        respond+="Done.\n";
        writeStringToClient(respond);
    }
};
